...
impl PtySession {
    /// Spawns the user's shell (`$SHELL`, falling back to `/bin/zsh`) inside
    /// a new PTY of the given size, with `TERM=xterm-256color` — the de
    /// facto baseline most CLI programs already target, without claiming
    /// support for terminal-specific extensions (e.g. Kitty's graphics
...
mod tests {
    ...
    fn spawn_write_read_roundtrip() {
        let mut session = PtySession::spawn(24, 80).expect("failed to spawn pty session");
